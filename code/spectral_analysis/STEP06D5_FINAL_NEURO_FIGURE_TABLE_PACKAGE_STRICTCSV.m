...
%
% Primary output:
%   Result/STEP06D5_Final_NeuroFigureTablePackage
%
% Inputs:
%   Result/STEP06C_HypothesisDriven_NeuralIndices
%
% Main outputs:
...
function Tdir = computeDirectionalConsistency(Tidx, Tcontrast)
    ...
    predictions = {
        ...
        'posterior_gamma_relative_power', 'orientation', 'orientation_dominant', 'orientation should be highest; conjunction may also exceed color'
        };
...
function Tpanel = makeFigurePanelPlan()
    ...
    rows(end+1,:) = {'Figure 1B', 'Task EEG index', 'Box/paired plot', ...
        ...
        'Shows orientation/conjunction-related relative gamma engagement, with orientation highest.'}; %#ok<AGROW>
    rows(end+1,:) = {'Figure 1C', 'Topographic organization', 'Box/paired plot', ...
        ...
        'Descriptive topographic index; no reliable overall condition effect in the final analysis.'}; %#ok<AGROW>
...
function txt = makeEnglishResults(TcondFinal, TcontrastFinal, TrtFinal)
    ...
    txt = sprintf(['Hypothesis-driven composite indices confirmed the main neuroscience pattern observed in the ROI-level analyses. ', ...
        ...
        'Maintenance posterior gamma relative power also differed across conditions, Friedman p = %.3g, Kendall''s W = %.2f, with the highest median value for orientation trials. ', ...
        'The posterior-minus-anterior maintenance alpha index was not interpreted as a positive effect in the final analysis; its omnibus values were Friedman p = %.3g, Kendall''s W = %.2f.\n\n', ...
        ...
        'Thus, posterior gamma is better described as an orientation/conjunction-related condition difference rather than a conjunction-specific marker.'], ...
...
function txt = makePersianResults(TcondFinal, TcontrastFinal, TrtFinal)
    ...
    txt = sprintf(['شاخص‌های ترکیبی فرضیه‌محور، الگوی اصلی نوروساینسی مشاهده‌شده در تحلیل‌های ROI را تأیید کردند. ', ...
        ...
        'شاخص توان نسبی گامای posterior در فاز maintenance نیز بین وضعیت‌ها متفاوت بود  Friedman p = %.3g, Kendall''s W = %.2f  و بیشترین میانه مربوط به وضعیت orientation بود. ', ...
        'شاخص آلفای posterior-minus-anterior در فاز maintenance در تحلیل نهایی به‌عنوان اثر مثبت تفسیر نشد  Friedman p = %.3g, Kendall''s W = %.2f.\n\n', ...
        ...
        'بنابراین، گامای posterior بهتر است به‌عنوان تفاوت مرتبط با شرایط orientation/conjunction و نه یک نشانگر اختصاصی conjunction توصیف شود.'], ...
        a.p_Friedman(1), a.KendallW(1), g.p_Friedman(1), g.KendallW(1), ap.p_Friedman(1), ap.KendallW(1), ...
        rt1.SpearmanRho(1), rt1.p_Spearman(1), rt2.SpearmanRho(1), rt2.p_Spearman(1), rt3.SpearmanRho(1), rt3.p_Spearman(1));
end
