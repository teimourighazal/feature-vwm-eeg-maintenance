%% STEP09A2_CONDITION_ERP_WINDOWS_N22_ROIFIX.m
% STEP09A: Condition-based phase-specific ERP component-window analysis.
%
% Goal:
%   Test whether color, orientation, and conjunction conditions differ in
%   ERP amplitude windows across stimulus, maintenance, and retrieval phases.
%
% Important:
%   This analysis does NOT use correct/incorrect labels.
%   It is designed for the final task-EEG N=22 sample.
%
% Outputs:
%   Result/STEP09A2_ConditionERP_Windows_N22 under the selected Article2 Analysis folder.
%
% Main outputs:
%   STEP09A2_DiagnosticSummary.csv
%   STEP09A2_MergedTrialPhaseTable.csv
%   STEP09A2_ERP_TrialComponentFeatures.csv
%   STEP09A2_ERP_SubjectConditionComponents.csv
%   STEP09A2_ConditionOmnibus_Friedman.csv
%   STEP09A2_ConditionPairwise_Signrank.csv
%   STEP09A2_AnalysisSummary.csv
...
outDir = fullfile(resultRoot, 'STEP09A2_ConditionERP_Windows_N22');

if ~exist(resultRoot, 'dir'); mkdir(resultRoot); end
if ~exist(outDir, 'dir'); mkdir(outDir); end

fprintf('\n=== STEP09A2 CONDITION-BASED ERP WINDOWS N22 - ROI FIX ===\n');
...
P.finalEEGSubjects = {'s1','s2','s3','s4','s5','s6','s7','s8','s9','s11','s12','s13','s14','s15','s16','s17','s18','s19','s20','s21','s22','s23'}';
...
diagnosticRows(end+1,:) = {'Best phase table found', double(~isempty(Tphase)), getInfoName(phaseInfo), getInfoPath(phaseInfo)}; %#ok<AGROW>
diagnosticRows(end+1,:) = {'Best raw path table found', double(~isempty(Traw)), getInfoName(rawInfo), getInfoPath(rawInfo)}; %#ok<AGROW>

if isempty(Tphase)
    Tdiag = cell2table(diagnosticRows, 'VariableNames', {'Metric','Value','TextValue','PathOrNote'});
    writetable(Tdiag, fullfile(outDir, 'STEP09A2_DiagnosticSummary.csv'));
    error('STEP09A cannot proceed: phase table not found.');
end
...
M = prepareConditionPhaseTable(Tphase, Traw, P, articleRoot, rootDir, outDir);

diagnosticRows(end+1,:) = {'Merged phase rows after filtering', height(M), '', ''}; %#ok<AGROW>
diagnosticRows(end+1,:) = {'Subjects after filtering', numel(unique(string(M.Subject))), strjoin(cellstr(unique(string(M.Subject),'stable')),'|'), ''}; %#ok<AGROW>
diagnosticRows(end+1,:) = {'Conditions after filtering', numel(unique(string(M.ConditionLabel))), strjoin(cellstr(unique(string(M.ConditionLabel),'stable')),'|'), ''}; %#ok<AGROW>

if isempty(M)
    Tdiag = cell2table(diagnosticRows, 'VariableNames', {'Metric','Value','TextValue','PathOrNote'});
    writetable(Tdiag, fullfile(outDir, 'STEP09A2_DiagnosticSummary.csv'));
    error('STEP09A cannot proceed: filtered phase table is empty.');
end

writetable(M, fullfile(outDir, 'STEP09A2_MergedTrialPhaseTable.csv'));

%% Trial counts
Tcounts = groupcounts(M, {'Subject','Run','ConditionLabel','PhaseLabel'});
Tcounts.Properties.VariableNames{end} = 'NTrialPhases';
writetable(Tcounts, fullfile(outDir, 'STEP09A2_TrialPhaseCounts.csv'));

%% ERP extraction
fprintf('\nExtracting ERP component-window features...\n');
Ttrial = extractERPFeatures09A(M, P, outDir);

if isempty(Ttrial)
    diagnosticRows(end+1,:) = {'STOP', 0, 'No ERP features extracted', ''}; %#ok<AGROW>
    Tdiag = cell2table(diagnosticRows, 'VariableNames', {'Metric','Value','TextValue','PathOrNote'});
    writetable(Tdiag, fullfile(outDir, 'STEP09A2_DiagnosticSummary.csv'));
    error('STEP09A stopped: no ERP features extracted.');
end

writetable(Ttrial, fullfile(outDir, 'STEP09A2_ERP_TrialComponentFeatures.csv'));

%% Subject-level aggregation and stats
Tsubj = aggregateSubjectConditionComponents09A(Ttrial, P.minTrialsPerSubjectCell);
writetable(Tsubj, fullfile(outDir, 'STEP09A2_ERP_SubjectConditionComponents.csv'));

Tomni = conditionOmnibusStats09A(Tsubj, P);
Tpair = conditionPairwiseStats09A(Tsubj, P);

writetable(Tomni, fullfile(outDir, 'STEP09A2_ConditionOmnibus_Friedman.csv'));
writetable(Tpair, fullfile(outDir, 'STEP09A2_ConditionPairwise_Signrank.csv'));

Tsummary = makeSummary09A(M, Ttrial, Tsubj, Tomni, Tpair);
writetable(Tsummary, fullfile(outDir, 'STEP09A2_AnalysisSummary.csv'));

diagnosticRows(end+1,:) = {'ERP trial-component rows', height(Ttrial), '', ''}; %#ok<AGROW>
diagnosticRows(end+1,:) = {'Subject-condition component rows', height(Tsubj), '', ''}; %#ok<AGROW>
diagnosticRows(end+1,:) = {'Omnibus tests', height(Tomni), '', ''}; %#ok<AGROW>
diagnosticRows(end+1,:) = {'Omnibus q<0.05', sum(Tomni.q_FDR < 0.05, 'omitnan'), '', ''}; %#ok<AGROW>
diagnosticRows(end+1,:) = {'Pairwise tests', height(Tpair), '', ''}; %#ok<AGROW>
diagnosticRows(end+1,:) = {'Pairwise q<0.05', sum(Tpair.q_FDR < 0.05, 'omitnan'), '', ''}; %#ok<AGROW>

Tdiag = cell2table(diagnosticRows, 'VariableNames', {'Metric','Value','TextValue','PathOrNote'});
writetable(Tdiag, fullfile(outDir, 'STEP09A2_DiagnosticSummary.csv'));

save(fullfile(outDir, 'STEP09A2_ConditionERP_Windows_N22_Workspace.mat'), ...
...
%% ===== STEP09A local functions =====

function M = prepareConditionPhaseTable(Tphase, Traw, P, articleRoot, rootDir, outDir)
    M = Tphase;

    M.Subject = normalizeSubjectKey(M.Subject);
    M.Run = normalizeRunKey(M.Run);
    M.Trial = normalizeTrialKey(M.Trial);

    if ismember('PhaseLabel', M.Properties.VariableNames)
        M.PhaseLabel = normalizePhaseLabel(M.PhaseLabel);
    end
    if ismember('ConditionLabel', M.Properties.VariableNames)
        M.ConditionLabel = normalizeConditionLabel(M.ConditionLabel);
    end
    ...
    if ~isempty(Traw) && ismember('RawEEGPath', Traw.Properties.VariableNames)
        try
            ...
            TrawSmall = unique(Traw(:, unique([rawKeys, {'RawEEGPath'}], 'stable')), 'rows');
    ...
    if all(strlength(string(M.RawEEGPath))==0 | ismissing(string(M.RawEEGPath)))
        fprintf('\nRaw EEG path missing; trying automatic raw-file mapping...\n');
        [M, autoRawMap, autoStatus] = attachAutoRawPaths(M, articleRoot, rootDir, outDir);
        fprintf('Auto raw-map status: %s\n', autoStatus);
        if ~isempty(autoRawMap)
            writetable(autoRawMap, fullfile(outDir, 'STEP09A2_AutoRawPathMap.csv'));
        end
    end

    M.RawEEGPath = resolveRawPaths(M.RawEEGPath, articleRoot, rootDir);

    keep = ismember(string(M.Subject), string(P.finalEEGSubjects)) & ...
           ismember(string(M.PhaseLabel), string(P.targetPhases)) & ...
           ismember(string(M.ConditionLabel), string(P.targetConditions)) & ...
           isfinite(M.PhaseStartSample) & M.PhaseStartSample > 0 & ...
           strlength(string(M.RawEEGPath)) > 0 & ~ismissing(string(M.RawEEGPath));

    M = M(keep,:);
end

function Ttrial = extractERPFeatures09A(M, P, outDir)
    rows = {};
            ...
            for c = 1:size(P.components,1)
                ...
                for rr = 1:numel(compROIs)
                    ...
                    rows(end+1,:) = { ...
                        ...
                        char(fileRows.ConditionLabel(r)), char(fileRows.PhaseLabel(r)), char(rawPath), ...
                        compName, roiUsed, winStart, winEnd, meanAmp, peakAmp, peakLatency, numel(chanIdx), fs}; %#ok<AGROW>
                end
            end
        end
    ...
    if ~isempty(logRows)
        ...
        writetable(Tlog, fullfile(outDir, 'STEP09A2_RawEEG_ReadLog.csv'));
    end
    ...
    if isempty(rows)
        ...
        Ttrial = cell2table(rows, 'VariableNames', ...
            {'Subject','Run','Trial','ConditionLabel','PhaseLabel','RawEEGPath', ...
            'Component','ROI','WindowStartMs','WindowEndMs','MeanAmplitude','PeakAmplitude','PeakLatencyMs','NChannels','SamplingRate'});
    end
end

function Tsubj = aggregateSubjectConditionComponents09A(Ttrial, minTrials)
...
function Tomni = conditionOmnibusStats09A(Tsubj, P)
    keys = {'PhaseLabel','Component','ROI'};
    U = unique(Tsubj(:,keys), 'rows');
    rows = {};
    for i = 1:height(U)
        S = Tsubj(strcmp(Tsubj.PhaseLabel,U.PhaseLabel{i}) & strcmp(Tsubj.Component,U.Component{i}) & strcmp(Tsubj.ROI,U.ROI{i}),:);
        W = wideConditions09A(S);
        ...
        rows(end+1,:) = {U.PhaseLabel{i}, U.Component{i}, U.ROI{i}, height(W), ...
    ...
    Tomni = cell2table(rows, 'VariableNames', ...
        {'PhaseLabel','Component','ROI','NSubjects','Median_color','Median_orientation','Median_conjunction','MaxCondition','p_Friedman','ChiSquare','KendallW','SubjectsUsed'});
    Tomni.q_FDR = fdrBH09(Tomni.p_Friedman);
    Tomni = sortrows(Tomni, {'q_FDR','p_Friedman'}, {'ascend','ascend'});
end

function Tpair = conditionPairwiseStats09A(Tsubj, P)
    ...
    for i = 1:height(U)
        ...
        for pp = 1:size(pairs,1)
            ...
            rows(end+1,:) = {U.PhaseLabel{i}, U.Component{i}, U.ROI{i}, [a '_vs_' b], height(W), ...
    ...
    Tpair = cell2table(rows, 'VariableNames', ...
        {'PhaseLabel','Component','ROI','Contrast','NSubjects','Median_A','Median_B','MedianDiff_AminusB','p_Signrank','Direction','SubjectsUsed'});
    Tpair.q_FDR = fdrBH09(Tpair.p_Signrank);
    Tpair = sortrows(Tpair, {'q_FDR','p_Signrank'}, {'ascend','ascend'});
end

function W = wideConditions09A(S)
    ...
    for i = 1:numel(subjects)
        ...
        for c = 1:3
            ...
            vals(c) = median(Sc.MedianMeanAmplitude,'omitnan');
        end
...
function Tsummary = makeSummary09A(M, Ttrial, Tsubj, Tomni, Tpair)
    rows = {};
    ...
    rows(end+1,:) = {'Subject-condition component rows', height(Tsubj)}; %#ok<AGROW>
...
function paths = resolveRawPaths(paths, articleRoot, rootDir)
    paths = string(paths);
    roots = {
...
function [M, rawMap, status] = attachAutoRawPaths(M, articleRoot, rootDir, outDir)
    ...
    % If raw recordings are stored outside the Article2 tree, allow the user
    % to select that folder without embedding a machine-specific path.
    if isempty(rawMap)
        rawRoot = uigetdir(pwd, 'Select folder containing raw task EEG MAT/EDF files');
        if ~isequal(rawRoot, 0)
            rawMap = buildAutoRawMap({rawRoot});
        end
    end
